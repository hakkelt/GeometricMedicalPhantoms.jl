...
function create_torso_phantom(nx::Int=128, ny::Int=128, nz::Int=128; fovs=(30, 30, 30), respiratory_signal=nothing, cardiac_volumes=nothing, ti::AbstractTissueParameters=TissueIntensities(), eltype=Float32)
    ...
    if is_mask
        phantom4d = falses(nx, ny, nz, nt)
        static_image = falses(nx, ny, nz)
    else
    ...
    draw_static_structures!(static_image, ax_xn, ax_yn, ax_zn, ti)
...
function create_torso_phantom(nx::Int, ny::Int, axis::Symbol; fovs=(30, 30), slice_position::Real=0.0, respiratory_signal=nothing, cardiac_volumes=nothing, ti::AbstractTissueParameters=TissueIntensities(), eltype=Float32)
    ...
    # 4) Draw static structures once
    draw_static_structures_2d!(static_image, ax_1n, ax_2n, ax_3_val, axis, ti)
...
"""
Helper function to define static phantom ellipsoids (structures that don't move).
Returns a tuple of all static superellipsoids.
"""
function define_static_ellipsoids(ti::AbstractTissueParameters)
    torso_static = get_torso_static_parts(ti)
    arm = get_arm_bones(ti)
    spine = get_spine(ti)
    return (torso_static..., arm..., spine...)
end

"""
Helper function to draw static phantom structures onto an image.
"""
function draw_static_structures!(image, ax_xn, ax_yn, ax_zn, ti::AbstractTissueParameters)
    static_ellipsoids = define_static_ellipsoids(ti)
    for se in static_ellipsoids
...
function define_dynamic_ellipsoids(motion_params::NamedTuple, ti::AbstractTissueParameters)
    ...
    lungs = get_lungs(motion_params.scale, motion_params.diaphragm_up, motion_params.diaphragm_rscale, 
                      motion_params.lower_rz_scale, motion_params.y_offset, ti)
    heart_background = get_heart_background(motion_params.heart_scale_max.lv, motion_params.heart_scale_max.rv, 
...
"""
function draw_static_structures_2d!(image, ax_1n, ax_2n, ax_3_val, axis::Symbol, ti::AbstractTissueParameters)
    static_ellipsoids = define_static_ellipsoids(ti)
    for se in static_ellipsoids
...
function get_ribs(rib_width_scale::Real, rib_depth_scale::Real, y_offset::Real, ti::AbstractTissueParameters)
    ...
    function create_rib_curve(z_pos, num_segments, torso_width, torso_depth, arc_coverage)
        ...
        for angle in angles
            ...
            parts = (parts..., SuperEllipsoid(x_pos, y_pos, z_pos + z_adjustment, 0.04, 0.04, 0.055, (2.0,2.0,2.0), get_intensity(ti, :bones)))
        end
    end
